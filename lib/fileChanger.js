...
module.exports = {
  wrapTemplate,
  wrapDirectiveFile,
  prependDirectiveFileContent,
  appendDirectiveFileContent,
  appendProvideGetter,
...
function wrapDirectiveFile(content, url) {
  return `\
${prependDirectiveFileContent(url)}\
${content}
${appendDirectiveFileContent()}
`;
}
...
function prependDirectiveFileContent(url) {
  const jsonUrl = JSON.stringify(url);
  return `try{window.___injular___.addScriptUrlToDirectives(${jsonUrl});`;
}


function appendDirectiveFileContent() {
  return `}finally{window.___injular___.currentDirectiveUrl = null};`;
}
